...
export async function getPackageDetail(name, version = 'latest') {
  ...
  // 2. Ambil semua list versi untuk menentukan mana yang 'latest'
  ...
  // 3. Tentukan path di storage (Contoh: "my-pkg/1.0.0")
  const basePath = `${name}/${finalVersion}`;

  // 4. Ambil flat list files dari storage
  ...
  // 6. Hitung atoms_count secara dinamis
  // Kita filter file yang jalurnya lewat folder "atoms/"
  const atomsCount = flatFiles.filter(f => f.path.includes(`${basePath}/atoms/`)).length;

  // 7. Return hasil akhir yang lengkap
  return {
    id: pkg.id,
    name: pkg.name,
    ...
    atoms_count: atomsCount, // Sekarang sudah real count!
    rank: (pkg.stars_count || 0) * 2 + (pkg.downloads_count || 0) / 2,
    tree: tree // Structure folder-file yang cantik
  };
}
